...
6. **确认默认深度档位（派单膨胀防线，2026-09-16 立）**：日常修复轮/小版本 → **增量档**（变更集内验证 + scope 判据照常，**不配预埋靶区清单**，目标墙钟 ≤10 分钟）；大版本手术、发布前、使用者明说「全面/深度」 → **全量档**（scope=全量 + 主会话可配定向靶区）。🔴 给子代理写派单时必须显式声明档位与是否配靶区——**默认不配**：靶区清单是主会话经验的临时外注，每次验收都靠人肉列靶区=8g 启发式没长成的信号，膨胀一次记一次，靶区沉淀进 8g 六形态/坑表后不再重配（例：-q 打架、闸门重排、开关门控翻案均已沉淀，同类无需再预埋）
